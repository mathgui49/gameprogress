...
export function TopBar() {
  ...
  gam.xpEvents.slice(0, 5).forEach((e) => {
    allNotifications.push({ id: `xp-${e.id}`, icon: "⚡", text: `+${e.amount} XP — ${e.reason}`, date: e.date, color: "text-[#c084fc]" });
  });
  ...
  pendingReceived.forEach((r) => {
    allNotifications.push({ id: `wing-${r.id}`, icon: "🤝", text: `Invitation wing recue`, date: r.createdAt, color: "text-[#818cf8]" });
  });
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between px-4 pt-4 lg:px-8 lg:pt-6">
        {/* Left: Profil, Parametres, Rapports */}
          ...
          {topLeftLinks.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                title={item.label}
                className={`relative p-2 rounded-xl transition-colors ${active ? "bg-[#c084fc]/15 text-[#c084fc]" : "text-[#6b6580] hover:text-[#a09bb2] hover:bg-[rgba(192,132,252,0.04)]"}`}
              >
        ...
        {/* Right: Calendar, Notifications, Account */}
        <div className="flex items-center gap-1">
          <Link
            href="/calendrier"
            title="Calendrier"
          ...
          {authSession?.user && (
            <button
              ...
              className="relative p-1 rounded-xl hover:bg-[rgba(192,132,252,0.04)] transition-colors"
            >
      ...
      {showAccount && authSession?.user && (
        <div className="fixed inset-0 z-50" onClick={() => setShowAccount(false)}>
          <div className="absolute top-14 right-4 lg:right-8 bg-[#14111c] border border-[rgba(192,132,252,0.12)] rounded-xl p-4 w-64 shadow-xl z-50" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-3">
              {authSession.user.image ? (
                <img src={authSession.user.image} alt="" className="w-10 h-10 rounded-full" referrerPolicy="no-referrer" />
              ) : (
                <div className="w-10 h-10 rounded-full bg-[#c084fc]/20 flex items-center justify-center text-sm font-bold text-[#c084fc]">
                  {authSession.user.name?.[0]?.toUpperCase() || "?"}
                </div>
              )}
              <div className="flex-1 min-w-0">
                <p className="text-sm text-white font-medium truncate">{authSession.user.name}</p>
                <p className="text-[10px] text-[#6b6580] truncate">{authSession.user.email}</p>
              </div>
            </div>
            <div className="border-t border-[rgba(192,132,252,0.08)] pt-3">
              <button
                onClick={() => signOut({ redirectTo: "/login" })}
                className="flex items-center gap-2 w-full text-left text-sm text-[#fb7185] hover:bg-[#fb7185]/10 rounded-lg px-3 py-2 transition-colors"
              >
          ...
          <div className="space-y-2 max-h-[60vh] overflow-y-auto">
            {notifications.length > 1 && (
              <button
                ...
                className="text-[10px] text-[#6b6580] hover:text-[#a09bb2] transition-colors mb-1"
              >
                Tout effacer
              </button>
            )}
                ...
                <div className="flex items-start gap-3">
                  ...
                  {!n.action && (
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 text-[#6b6580] hover:text-[#a09bb2] transition-all shrink-0"
                      title="Supprimer"
                    >
