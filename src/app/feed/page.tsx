    ...
    return (
      <div className="flex items-center gap-2.5 mb-3">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#c084fc]/20 to-[#818cf8]/20 flex items-center justify-center">
          <span className="text-xs font-bold text-[#c084fc]">{profile.firstName?.[0]?.toUpperCase() || "?"}</span>
        </div>
        ...
        {badge && <Badge className="bg-[#818cf8]/15 text-[#818cf8]">{badge}</Badge>}
        <span className="text-[10px] text-[#6b6580] shrink-0">{formatRelative(createdAt)}</span>
      </div>
    );
  };
  ...
  return (
    <div className="px-4 py-6 lg:px-8 lg:py-8 max-w-2xl mx-auto animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-sm text-[#a09bb2]">Activite de la communaute et de tes wings</p>
      </div>
            ...
            <div key={`${item.type}-${item.data.id}`} className="animate-slide-up" style={{ animationDelay: `${idx * 40}ms` }}>
              ...
              {item.type === "session" && (
                <Card className="!p-4">
                  ...
                  <div className="mb-3">
                    <p className="text-sm font-semibold text-white">{item.data.title || "Session"}</p>
                    ...
                    {item.data.notes && <p className="text-xs text-[#a09bb2] mt-2 line-clamp-3">{item.data.notes}</p>}
                  </div>
                  <div className="flex items-center gap-4 pt-2 border-t border-[rgba(192,132,252,0.06)]">
                    <button onClick={() => handleLike(item.data.id)} className="flex items-center gap-1.5 text-[#6b6580] hover:text-[#f472b6] transition-colors">
                    ...
                    <button onClick={() => openComments(item.data.id)} className="flex items-center gap-1.5 text-[#6b6580] hover:text-[#c084fc] transition-colors">
                    ...
                    <div className="mt-3 pt-3 border-t border-[rgba(192,132,252,0.06)] space-y-2">
                      {comments.map((c) => (
                        <div key={c.id} className="flex gap-2">
                          ...
                          <div>
                            <p className="text-[10px] text-[#6b6580]">{c.profile?.firstName || "Anonyme"} · {formatRelative(c.createdAt)}</p>
              ...
              {item.type === "journal" && (
                <Card className="!p-4">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <svg className="w-4 h-4 text-[#f472b6]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                    <span className="text-xs text-[#f472b6] font-medium">Journal</span>
                  ...
                  <p className="text-sm text-[#a09bb2] leading-relaxed line-clamp-5">{item.data.content}</p>
                </Card>
              )}
              ...
              {item.type === "post" && (
                <Card className="!p-4">
                  {renderAuthor(item.profile, item.createdAt, item.data.visibility === "wings" ? "Wings" : undefined)}
                  <p className="text-sm text-[#a09bb2] leading-relaxed">{item.data.content}</p>
                </Card>
              )}
            </div>
          ))}
