  ...
  return (
    <div className="px-4 py-6 lg:px-8 lg:py-8 max-w-4xl mx-auto animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl lg:text-3xl font-[family-name:var(--font-grotesk)] font-bold text-white tracking-tight mb-1">Interactions</h1>
          <p className="text-sm text-[#a09bb2]">{interactions.length} interaction{interactions.length !== 1 ? "s" : ""}</p>
        </div>
        <Link href="/interactions/new"><Button>+ Ajouter</Button></Link>
      </div>

      {interactions.length > 0 && (
        <div className="flex items-center gap-1 p-1 bg-[#100e17] rounded-xl w-fit mb-4">
          {(["newest", "oldest"] as const).map((o) => (
            <button key={o} onClick={() => setSortOrder(o)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${sortOrder === o ? "bg-white/[0.06] text-white" : "text-[#6b6580] hover:text-[#a09bb2]"}`}>
