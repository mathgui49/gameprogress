...
export default function InteractionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!interaction) return <div className="flex flex-col items-center justify-center h-screen"><p className="text-[#a09bb2] mb-4">Interaction introuvable</p><Button variant="secondary" onClick={() => router.push("/interactions")}>Retour</Button></div>;

  if (editing) {
    return (
      <div className="px-4 py-6 lg:px-8 lg:py-8 max-w-2xl mx-auto animate-fade-in">
        <button onClick={() => setEditing(false)} className="flex items-center gap-1 text-sm text-[#a09bb2] hover:text-white transition-colors mb-4">
  ...
  return (
    <div className="px-4 py-6 lg:px-8 lg:py-8 max-w-3xl mx-auto animate-fade-in">
      <button onClick={() => router.push("/interactions")} className="flex items-center gap-1 text-sm text-[#a09bb2] hover:text-white transition-colors mb-6">
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg>
        Interactions
      </button>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          {interaction.firstName && interaction.memorableElement && (
            <p className="text-sm text-[#a09bb2] mb-2 italic">{interaction.memorableElement}</p>
          )}
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-xs text-[#6b6580]">{formatDate(interaction.date)}</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
        <Card className="text-center !p-4">
          <p className="text-[10px] text-[#6b6580] uppercase tracking-wider mb-1">Ressenti</p>
          <p className="text-2xl font-bold text-[#c084fc]">{interaction.feelingScore}<span className="text-sm text-[#6b6580]">/10</span></p>
        </Card>
        <Card className="text-center !p-4">
          <p className="text-[10px] text-[#6b6580] uppercase tracking-wider mb-1">Note fille</p>
          <p className="text-2xl font-bold text-[#818cf8]">{interaction.womanScore ?? "-"}<span className="text-sm text-[#6b6580]">/10</span></p>
        </Card>
        <Card className="text-center !p-4">
          <p className="text-[10px] text-[#6b6580] uppercase tracking-wider mb-1">Confiance</p>
          <p className="text-2xl font-bold text-cyan-400">{interaction.confidenceScore ?? "-"}<span className="text-sm text-[#6b6580]">/10</span></p>
        </Card>
        <Card className="text-center !p-4">
          <p className="text-[10px] text-[#6b6580] uppercase tracking-wider mb-1">Duree</p>
          <p className="text-sm font-semibold text-white">{DURATION_LABELS[interaction.duration]}</p>
        </Card>
        <Card className="text-center !p-4">
          <p className="text-[10px] text-[#6b6580] uppercase tracking-wider mb-1">Type</p>
      ...
      {interaction.location && (
        <Card className="mb-4 !p-4">
          <div className="flex items-center gap-2 text-sm">
            <svg className="w-4 h-4 text-[#6b6580]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" />
              <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" />
            </svg>
            <span className="text-[#a09bb2]">{interaction.location}</span>
          </div>
        </Card>
      )}
      ...
      {interaction.discussionTopics && (
        <Card className="mb-4">
          <h3 className="text-xs font-semibold text-[#a09bb2] uppercase tracking-wider mb-2">Sujets de discussion</h3>
          <p className="text-sm text-[#a09bb2] leading-relaxed whitespace-pre-wrap">{interaction.discussionTopics}</p>
        </Card>
      )}
      ...
      {interaction.note && (
        <Card className="mb-4">
          <h3 className="text-xs font-semibold text-[#a09bb2] uppercase tracking-wider mb-2">Notes</h3>
          <p className="text-sm text-[#a09bb2] leading-relaxed whitespace-pre-wrap">{interaction.note}</p>
        </Card>
      )}
      ...
      {interaction.feedback && (
        <Card className="mb-4">
          <h3 className="text-xs font-semibold text-[#a09bb2] uppercase tracking-wider mb-2">Feedback personnel</h3>
          <p className="text-sm text-[#a09bb2] leading-relaxed whitespace-pre-wrap">{interaction.feedback}</p>
        </Card>
      )}

      <Modal open={showDelete} onClose={() => setShowDelete(false)} title="Supprimer">
        <p className="text-sm text-[#a09bb2] mb-6">Es-tu sur ? Cette action est irreversible.</p>
