...
import { JOURNAL_TAG_LABELS, JOURNAL_TAG_COLORS, DEFAULT_PRIVACY } from "@/types";
...
export default function WingProfilePage() {
  ...
  return (
    <div className="px-4 py-6 lg:px-8 lg:py-8 max-w-3xl mx-auto animate-fade-in">
      ...
      {/* Badges — from BADGE_CATEGORIES tiers */}
      {(() => {
        // Show highest unlocked tier per category as profile badges
        const earnedTiers = BADGE_CATEGORIES.map((cat) => {
          // Try to derive value from gamification data
          const val = cat.key === "level" ? gam.level : cat.key === "streak" ? gam.streak : 0;
          // For profile view, we check legacy badges as fallback
          let highestTier = null;
          for (let i = cat.tiers.length - 1; i >= 0; i--) {
            // Check if any legacy badge matching this category is unlocked
            const legacyMatch = badges.find((b) => b.unlockedAt && b.name.toLowerCase().includes(cat.tiers[i].name.toLowerCase()));
            if (legacyMatch || (val > 0 && val >= cat.tiers[i].threshold)) {
              highestTier = { ...cat.tiers[i], catLabel: cat.label };
              break;
            }
          }
          return highestTier;
        ...
        // Also show legacy unlocked badges not covered by categories
        const legacyUnlocked = badges.filter((b) => b.unlockedAt);

        const displayBadges = earnedTiers.length > 0 ? earnedTiers : legacyUnlocked;
        if (displayBadges.length === 0) return null;

        return (
          <Card className="mb-4">
            <h2 className="text-sm font-semibold text-[var(--on-surface)] mb-3">Badges ({displayBadges.length})</h2>
            <div className="flex flex-wrap gap-2">
              {earnedTiers.length > 0 ? earnedTiers.map((tier: any, i: number) => (
                <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[var(--surface-bright)] border border-[var(--border)]">
                  <BadgeIcon icon={tier.icon} size={16} />
                  <div>
                    <p className="text-xs font-medium text-[var(--on-surface)]">{tier.name}</p>
                    <p className="text-[9px] text-[var(--outline)]">{tier.catLabel}</p>
                  </div>
                </div>
              )) : legacyUnlocked.map((b: BadgeType) => (
